...
second-least-significant one-limb byte-pack block within
`mloadFourLimbsCode`) to the program-identical
`evm_calldataload_window_code`.

Mirrors `calldataload_window_one_limb_q0_stack_spec_within` for the q1
quarter; q2/q3 land in followup sub-slices.
