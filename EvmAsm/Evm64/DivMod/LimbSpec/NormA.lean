...
theorem divK_normA_top_spec (src_off dst_off : BitVec 12)
  ...
  (divK_normA_top_spec_within src_off dst_off sp val v5 v7 antiShift dstOld base).to_cpsTriple
...
theorem divK_normA_mergeA_spec (next_off dst_off : BitVec 12)
    ...
    cpsTriple base (base + 20) cr
      ...
      (
       ...
       ((sp + signExtend12 next_off) ↦ₘ next) **
       ((sp + signExtend12 dst_off) ↦ₘ result)) :=
  (divK_normA_mergeA_spec_within next_off dst_off sp current next v7 v10 shift antiShift dstOld base).to_cpsTriple
...
theorem divK_normA_mergeB_spec (next_off dst_off : BitVec 12)
    ...
    cpsTriple base (base + 20) cr
      ...
      (
       ...
       ((sp + signExtend12 next_off) ↦ₘ next) **
       ((sp + signExtend12 dst_off) ↦ₘ result)) :=
  (divK_normA_mergeB_spec_within next_off dst_off sp current next v5 v10 shift antiShift dstOld base).to_cpsTriple
...
theorem divK_normA_last_spec (dst_off : BitVec 12)
    ...
    cpsTriple base (base + 8) cr
      ...
      (
       (.x12 ↦ᵣ sp) ** (.x7 ↦ᵣ result) ** (.x6 ↦ᵣ shift) **
       ((sp + signExtend12 dst_off) ↦ₘ result)) :=
  (divK_normA_last_spec_within dst_off sp val shift dstOld base).to_cpsTriple
