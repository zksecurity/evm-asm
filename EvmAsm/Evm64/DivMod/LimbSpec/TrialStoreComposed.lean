...
theorem divK_trial_load_spec
    ...
    cpsTriple base (base + 48) cr
      ...
      ((.x12 ↦ᵣ sp) ** (.x1 ↦ᵣ j) **
       ...
       (uAddr ↦ₘ uHi) ** ((uAddr + 8) ↦ₘ uLo) **
       (vtopBase + signExtend12 32 ↦ₘ vTop)) :=
  (divK_trial_load_spec_within sp j n v5Old v6Old v7Old v10Old uHi uLo vTop base).to_cpsTriple
...
theorem divK_store_qj_spec (sp j qHat v5Old v7Old qOld : Word)
    ...
    cpsTriple base (base + 16) cr
      ...
      ((.x1 ↦ᵣ j) ** (.x12 ↦ᵣ sp) ** (.x11 ↦ᵣ qHat) **
       (.x5 ↦ᵣ jX8) ** (.x7 ↦ᵣ qAddr) **
       (qAddr ↦ₘ qHat)) :=
  (divK_store_qj_spec_within sp j qHat v5Old v7Old qOld base).to_cpsTriple
