...
/-- No-NOP/v4 loop body cpsTripleWithin for n=1, max+addback, j=0. -/
theorem divK_loop_body_n1_max_addback_j0_beq_v4_spec_within_noNop
  ...
  have TF := divK_trial_max_full_v4_spec_within_noNop sp (0 : Word) (1 : Word)
    jOld v5Old v6Old v7Old v10Old v11Old u1 u0 v0 base hbltu
  dsimp only [] at TF
  ...
  exact cpsTripleWithin_weaken
    ...
    (fun h hp => by
      delta loopBodyN1AddbackBeqPost loopBodyAddbackBeqPost loopExitPostN1 loopExitPost
      rw [sepConj_assoc'] at hp; xperm_hyp hp)
    full

/-- No-NOP/v4 loop body cpsTripleWithin for n=1, max+addback, j>0. -/
theorem divK_loop_body_n1_max_addback_jgt0_beq_v4_spec_within_noNop (j : Word)
  ...
  let vtopBase := sp + ((1 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_max_full_v4_spec_within_noNop sp j (1 : Word)
    jOld v5Old v6Old v7Old v10Old v11Old u1 u0 v0 base hbltu
  dsimp only [] at TF
  rw [u_addr_eq_n1] at TF
  rw [u_addr8_eq_n1] at TF
