...
/-- Framed variant of
    `evm_div_callable_spec_from_noNop_preserving_x1_noX9`. -/
theorem evm_div_callable_spec_from_noNop_preserving_x1_noX9_framed
    ...
    cpsTripleWithin (unifiedDivBound + 1) base (raVal &&& ~~~1)
      ...
      (divModStackDispatchPreCallable sp a b
        ...
        q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
        shiftMem nMem jMem retMem dMem dloMem scratchUn0 ** F)
      ((divStackDispatchPostCallable sp a b ** (.x1 ↦ᵣ raVal)) ** F) := by
  exact
    cpsTripleWithin_frameR F (by pcFree)
      (evm_div_callable_spec_from_noNop_preserving_x1_noX9
...
/-- Generic callable DIV wrapper for no-NOP body proofs that already use the
    ...
    Branch-local callable dispatcher proofs can target this theorem directly:
    the no-NOP body proves the DIV computation up to `base + nopOff`, then
    this wrapper appends the callable return instruction. -/
theorem evm_div_callable_spec_from_noNop_preserving_x1_x9
    (sp base x9Val raVal : Word) (a b : EvmWord)
    (v2 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
     nMem shiftMem jMem retMem dMem dloMem scratchUn0 : Word)
...
/-- Framed variant of
    `evm_div_callable_spec_from_noNop_preserving_x1_x9`. -/
theorem evm_div_callable_spec_from_noNop_preserving_x1_x9_framed
    {F : Assertion} [Assertion.PCFree F]
    (sp base x9Val raVal : Word) (a b : EvmWord)
    (v2 v5 v6 v7 v10 v11 : Word)
    (q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
