...
theorem loopBodyN3MaxSkipPre_unfold
    ...
    (let uBase := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
     ...
     ((uBase + signExtend12 4064) ↦ₘ uTop) **
     (qAddr ↦ₘ qOld)) := by
  delta loopBodyN3MaxSkipPre; rfl
...
/-- No-NOP variant of `divK_loop_body_n3_max_skip_j0_spec_within`. -/
theorem divK_loop_body_n3_max_skip_j0_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (base : Word)
    (hbltu : ¬BitVec.ult u3 v2) :
  ...
  let u4_new := uTop - c3
  let vtopBase := sp + ((3 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_max_full_spec_within_noNop sp (0 : Word) (3 : Word) jOld v5Old v6Old v7Old v10Old v11Old
    u3 u2 v2 base hbltu
  dsimp only [] at TF
  rw [u_addr_eq_n3] at TF
  rw [u_addr8_eq_n3] at TF
...
@[irreducible]
def loopBodyN3CallPre
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     j v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (retMem dMem dloMem scratchUn0 : Word) : Assertion :=
  let uBase := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
  let qAddr := sp + signExtend12 4088 - j <<< (3 : BitVec 6).toNat
  (.x12 ↦ᵣ sp) ** (.x9 ↦ᵣ j) **
  (.x5 ↦ᵣ v5Old) ** (.x6 ↦ᵣ v6Old) **
...
theorem loopBodyN3CallPre_unfold
    ...
    loopBodyN3CallPre sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
      j v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld retMem dMem dloMem scratchUn0 =
    (let uBase := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
     let qAddr := sp + signExtend12 4088 - j <<< (3 : BitVec 6).toNat
     (.x12 ↦ᵣ sp) ** (.x9 ↦ᵣ j) **
     (.x5 ↦ᵣ v5Old) ** (.x6 ↦ᵣ v6Old) **
     ...
     (sp + signExtend12 3960 ↦ₘ dMem) **
     (sp + signExtend12 3952 ↦ₘ dloMem) **
     (sp + signExtend12 3944 ↦ₘ scratchUn0) ** regOwn .x1) := by
  delta loopBodyN3CallPre; rfl

/-- j-parameterized call-skip precondition (same layout as `loopBodyN3CallPre`). -/
@[irreducible]
def loopBodyN3CallSkipPre
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     j v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (retMem dMem dloMem scratchUn0 : Word) : Assertion :=
  let uBase := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
...
theorem loopBodyN3CallSkipPre_unfold
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     j v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (retMem dMem dloMem scratchUn0 : Word) :
    ...
    (let uBase := sp + signExtend12 4056 - j <<< (3 : BitVec 6).toNat
     ...
     (sp + signExtend12 3952 ↦ₘ dloMem) **
     (sp + signExtend12 3944 ↦ₘ scratchUn0) ** regOwn .x1) := by
  delta loopBodyN3CallSkipPre; rfl
...
theorem divK_loop_body_n3_call_skip_j0_spec_within
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by rw [loopBodyN3CallPre_unfold] at hp; xperm_hyp hp)
    (fun h hp => by
...
/-- No-NOP variant of `divK_loop_body_n3_call_skip_j0_spec_within`. -/
theorem divK_loop_body_n3_call_skip_j0_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (retMem dMem dloMem scratch_un0 : Word)
    (base : Word)
  ...
  unfold isSkipBorrowN3Call div128Quot at hborrow
  let vtopBase := sp + ((3 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_call_full_spec_within_noNop sp (0 : Word) (3 : Word) jOld v5Old v6Old v7Old v10Old v11Old v2Old
    u3 u2 v2 retMem dMem dloMem scratch_un0 base
    halign hbltu
  unfold divKTrialCallFullPost at TF
  dsimp only [] at TF
...
theorem divK_loop_body_n3_max_skip_j1_spec_within
    ...
    cpsTripleWithin 76 (base + loopBodyOff) (base + loopBodyOff) (sharedDivModCode base)
      ...
      (loopBodyN3SkipPost sp (1 : Word) (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 uTop) := by
  intro hborrow
...
/-- No-NOP variant of `divK_loop_body_n3_max_skip_j1_spec_within`. -/
theorem divK_loop_body_n3_max_skip_j1_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (base : Word)
    (hbltu : ¬BitVec.ult u3 v2) :
  ...
  let u4_new := uTop - c3
  let vtopBase := sp + ((3 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_max_full_spec_within_noNop sp (1 : Word) (3 : Word) jOld v5Old v6Old v7Old v10Old v11Old
    u3 u2 v2 base hbltu
  dsimp only [] at TF
  rw [u_addr_eq_n3] at TF
  rw [u_addr8_eq_n3] at TF
  rw [vtop_eq_v2_n3] at TF
  have MCS := divK_mulsub_correction_skip_spec_within_noNop sp qHat (1 : Word) v0 v1 v2 v3 u0 u1 u2 u3 uTop
    (1 : Word) u2 vtopBase u3 v2 v2Old base
  intro_lets at MCS
  have MCS0 := MCS hborrow
  have SL := divK_store_loop_jgt0_spec_within_noNop sp (1 : Word) qHat u4_new (0 : Word) qOld base slt_jpos_1
  intro_lets at SL
  have TFf := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ v2Old) **
     ((sp + signExtend12 32) ↦ₘ v0) ** ((uBase + signExtend12 0) ↦ₘ u0) **
...
/-- No-NOP variant of `divK_loop_body_n3_call_skip_j1_spec_within`. -/
theorem divK_loop_body_n3_call_skip_j1_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (retMem dMem dloMem scratch_un0 : Word)
    (base : Word)
  ...
  unfold isSkipBorrowN3Call div128Quot at hborrow
  let vtopBase := sp + ((3 : Word) + signExtend12 4095) <<< (3 : BitVec 6).toNat
  have TF := divK_trial_call_full_spec_within_noNop sp (1 : Word) (3 : Word) jOld v5Old v6Old v7Old v10Old v11Old v2Old
    u3 u2 v2 retMem dMem dloMem scratch_un0 base
    halign hbltu
  unfold divKTrialCallFullPost at TF
  dsimp only [] at TF
...
/-- No-NOP variant of `divK_loop_body_n3_max_addback_beq_j0_spec_within`. -/
theorem divK_loop_body_n3_max_addback_beq_j0_spec_within_noNop
    (sp jOld v5Old v6Old v7Old v10Old v11Old v2Old
     v0 v1 v2 v3 u0 u1 u2 u3 uTop qOld : Word)
    (base : Word)
    (hbltu : ¬BitVec.ult u3 v2)
  ...
  rw [u_addr8_eq_n3] at TF
  rw [vtop_eq_v2_n3] at TF
  have MCA := divK_mulsub_correction_addback_beq_spec_within_noNop sp qHat (0 : Word) v0 v1 v2 v3 u0 u1 u2 u3 uTop
    (0 : Word) u2 vtopBase u3 v2 v2Old base
  intro_lets at MCA
  have MCA0 := MCA hcarry2_nz hborrow
...
theorem divK_loop_body_n3_max_addback_beq_j1_spec_within
    ...
    cpsTripleWithin 152 (base + loopBodyOff) (base + loopBodyOff) (sharedDivModCode base)
      ((.x12 ↦ᵣ sp) ** (.x9 ↦ᵣ (1 : Word)) **
       ...
       ((uBase + signExtend12 4064) ↦ₘ uTop) **
       (qAddr ↦ₘ qOld))
      (loopBodyN3AddbackBeqPost sp (1 : Word) (signExtend12 4095 : Word) v0 v1 v2 v3 u0 u1 u2 u3 uTop) := by
  intro uBase qAddr hborrow
  let qHat : Word := signExtend12 4095
  unfold isAddbackCarry2NzN3Max isAddbackCarry2Nz at hcarry2_nz
  -- Named-function lets (NOT inline expansion)
