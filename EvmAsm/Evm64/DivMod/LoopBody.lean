...
theorem divK_mulsub_4limbs_spec_within
    ...
    let un3 := u3 - fs3
    let c3 := pc3 + bs3
    cpsTripleWithin 44 (base + 536) (base + correctionAddbackOff) (sharedDivModCode base)
...
theorem divK_mulsub_4limbs_v2_spec_within
    ...
    let un3 := u3 - fs3
    let c3 := pc3 + bs3
    cpsTripleWithin 44 (base + 536) (base + correctionAddbackOff) (sharedDivModCode_v2 base)
      ((.x12 ↦ᵣ sp) ** (.x11 ↦ᵣ qHat) ** (.x10 ↦ᵣ (signExtend12 0 : Word)) **
       (.x6 ↦ᵣ uBase) ** (.x5 ↦ᵣ v5_init) ** (.x7 ↦ᵣ v7_init) **
       (.x2 ↦ᵣ v2_init) **
       ((sp + signExtend12 32) ↦ₘ v0) ** ((uBase + signExtend12 0) ↦ₘ u0) **
