...
/-- No-NOP variant of `evm_div_phaseAB_n3_clz_spec_within`. -/
theorem evm_div_phaseAB_n3_clz_spec_within_noNop (sp base : Word)
  ...
  have hAf := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) **
     ...
     ((sp + signExtend12 4000) ↦ₘ u7) ** ((sp + signExtend12 3984) ↦ₘ nMem))
    (by pcFree) hA
  have hB := evm_div_phaseB_n3_spec_within_noNop sp base b1 b2 b3
    (b0 ||| b1 ||| b2 ||| b3) v6 v7 q0 q1 q2 q3 u5 u6 u7 nMem
    hb3z hb2nz
  have hBf := cpsTripleWithin_frameR
    (((sp + 32) ↦ₘ b0))
  ...
  have hAB := cpsTripleWithin_seq_perm_same_cr
    (fun h hp => by xperm_hyp hp) hAf hBf
  have hCLZ := divK_clz_spec_within_noNop b2 b1 b2 base
  have hCLZf := cpsTripleWithin_frameR
    ((.x12 ↦ᵣ sp) ** (.x10 ↦ᵣ b3) **
     ((sp + 32) ↦ₘ b0) ** ((sp + 40) ↦ₘ b1) **
     ((sp + 48) ↦ₘ b2) ** ((sp + 56) ↦ₘ b3) **
...
/-- PhaseAB(n=3) + CLZ + PhaseC2(shift≠0) + NormB over `divCode_noNop`.
    base → base+312. b[0..3] normalized in-place. -/
theorem evm_div_n3_to_normB_spec_within_noNop (sp base : Word)
    (b0 b1 b2 b3 v5 v6 v7 v10 : Word)
  ...
  have hABCLZ := evm_div_phaseAB_n3_clz_spec_within_noNop sp base b0 b1 b2 b3
    v5 v6 v7 v10 q0 q1 q2 q3 u5 u6 u7 nMem hbnz hb3z hb2nz
  have hABCLZf := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ (clzResult b2).2 >>> (63 : Nat)) **
     ((sp + signExtend12 3992) ↦ₘ shiftMem))
    (by pcFree) hABCLZ
  have hC2 := divK_phaseC2_ntaken_spec_within_noNop sp shift ((clzResult b2).2 >>> (63 : Nat))
...
theorem evm_div_n3_to_loopSetup_spec_within_noNop (sp base : Word)
  ...
  have hLS := divK_loopSetup_ntaken_spec_within_noNop sp (3 : Word)
