...
/-- DIV PhaseAB(n=1) + CLZ over `divCode_noNop_v4`.
    b≠0, b[3]=b[2]=b[1]=0; base → base+212. -/
theorem evm_div_phaseAB_n1_clz_spec_v4_noNop (sp base : Word)
  ...
  have hAB := evm_div_phaseAB_n1_spec_within_v4_noNop sp base b0 b1 b2 b3 v5 v6 v7 v10
    q0 q1 q2 q3 u5 u6 u7 nMem hbnz hb3z hb2z hb1z
  have hCLZ := divK_clz_spec_within_v4_noNop b0 b1 b2 base
...
/-- Full n=4 path from entry to loop body start over `divCode_noNop_v4`
    (shift ≠ 0 case). Normalizes b[] and a[], then sets up loop parameters. -/
theorem evm_div_n4_to_loopSetup_spec_v4_noNop (sp base : Word)
  ...
  have hNormB := evm_div_n4_to_normB_spec_v4_noNop sp base b0 b1 b2 b3 v5 v6 v7 v10
  ...
  have hLS := divK_loopSetup_ntaken_spec_within_v4_noNop sp (4 : Word)
...
/-- Full n=4 path from entry to loop body start over `divCode_noNop_v4`
    (shift = 0 case). b[] is already normalized, and u[] is copied from a[]. -/
theorem evm_div_n4_shift0_to_loopSetup_spec_v4_noNop (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
    (hb3nz : b3 ≠ 0)
    (hshift_z : (clzResult b3).1 = 0) :
    cpsTripleWithin (8 + 21 + 24 + 4 + 9 + 4) base (base + loopBodyOff) (divCode_noNop_v4 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) ** (.x10 ↦ᵣ v10) ** (.x0 ↦ᵣ (0 : Word)) **
       (.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) ** (.x2 ↦ᵣ (clzResult b3).2 >>> (63 : Nat)) **
       (.x9 ↦ᵣ signExtend12 (4 : BitVec 12) - (4 : Word)) **
       ((sp + 0) ↦ₘ a0) ** ((sp + 8) ↦ₘ a1) **
  ...
  have hABCLZf := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ (clzResult b3).2 >>> (63 : Nat)) **
     ...
     ((sp + signExtend12 3992) ↦ₘ shiftMem))
    (by pcFree) hABCLZ
  have hC2 := divK_phaseC2_taken_spec_within_v4_noNop sp ((clzResult b3).1)
  ...
  have hABC2 := cpsTripleWithin_seq_perm_same_cr
    (fun h hp => by xperm_hyp hp) hABCLZf hC2f
  have hCopy := divK_copyAU_full_spec_within_v4_noNop sp a0 a1 a2 a3
