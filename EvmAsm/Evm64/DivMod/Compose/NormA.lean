...
/-- NormA code (block 5) is subsumed by divCode_noNop_v4. -/
private theorem divK_normA_code_sub_divCode_noNop_v4 {base : Word} :
    ∀ a i, (CodeReq.ofProg (base + normAOff) (divK_normA 40)) a = some i →
      (divCode_noNop_v4 base) a = some i := by
  intro a i h
  exact sharedNoNop_v4_b5_div a i h
...
theorem divK_normA_full_spec_within_v4_noNop (sp a0 a1 a2 a3 v5 v7 v10 shift antiShift : Word)
    (u0Old u1Old u2Old u3Old u4Old : Word) (base : Word) :
    cpsTripleWithin 21 (base + normAOff) (base + loopSetupOff) (divCode_noNop_v4 base)
      (divKNormAFullPreNoNop sp v5 v7 v10 shift antiShift
        a0 a1 a2 a3 u0Old u1Old u2Old u3Old u4Old)
      (normAFullPost sp a0 a1 a2 a3 shift antiShift) := by
  rw [divKNormAFullPreNoNop_unfold, normAFullPost_unfold]
  ...
  simp only [se12_24] at htop
  have htope := cpsTripleWithin_extend_code (hmono := fun a i h =>
    divK_normA_code_sub_divCode_noNop_v4 a i
      (CodeReq.ofProg_mono_sub (base + normAOff) (base + normAOff) (divK_normA 40)
        (divK_normA_top_prog 24 4024) 0
        (by bv_addr) (by decide) (by decide) (by decide) a i h)) htop
  have htopef := cpsTripleWithin_frameR
  ...
  have hjale := cpsTripleWithin_extend_code (hmono := by
    intro a i h
    exact divK_normA_code_sub_divCode_noNop_v4 a i
      (CodeReq.singleton_mono (by
        have hlookup := CodeReq.ofProg_lookup (base + normAOff) (divK_normA 40) 20
          (by decide) (by decide)
        rw [show (base + normAOff : Word) + BitVec.ofNat 64 (4 * 20) = base + normAOff + 80 from by bv_addr]
  ...
  let postAll := (.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ u1) ** (.x7 ↦ᵣ u0) **
    ...
    ((sp + signExtend12 4056) ↦ₘ u0)
  have hjalef := cpsTripleWithin_frameR postAll (by pcFree) hjale
  have hjal_clean : cpsTripleWithin 1 (base + normAOff + 80) (base + loopSetupOff) (divCode_noNop_v4 base) postAll postAll :=
    cpsTripleWithin_weaken
      (fun h hp => by show (empAssertion ** postAll) h; rw [sepConj_emp_left']; exact hp)
      (fun h hp => by rw [sepConj_emp_left'] at hp; exact hp)
      hjalef
...
/-- CopyAU code (block 6) is subsumed by divCode_noNop_v4. -/
private theorem divK_copyAU_code_sub_divCode_noNop_v4 {base : Word} :
    ∀ a i, (divK_copyAU_code (base + copyAUOff)) a = some i →
      (divCode_noNop_v4 base) a = some i := by
  intro a i h
  exact sharedNoNop_v4_b6_div a i h
...
theorem divK_copyAU_full_spec_within_v4_noNop (sp : Word)
    (a0 a1 a2 a3 : Word) (u0 u1 u2 u3 u4 v5 : Word) (base : Word) :
    cpsTripleWithin 9 (base + copyAUOff) (base + loopSetupOff) (divCode_noNop_v4 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) **
       ((sp + signExtend12 0) ↦ₘ a0) ** ((sp + 8) ↦ₘ a1) **
       ((sp + 16) ↦ₘ a2) ** ((sp + 24) ↦ₘ a3) **
       ((sp + signExtend12 4056) ↦ₘ u0) ** ((sp + signExtend12 4048) ↦ₘ u1) **
  ...
  have hcopy := divK_copyAU_spec_within sp (base + copyAUOff) a0 a1 a2 a3 u0 u1 u2 u3 u4 v5
  rw [show (base + copyAUOff : Word) + 36 = base + loopSetupOff from by bv_addr] at hcopy
  exact cpsTripleWithin_extend_code divK_copyAU_code_sub_divCode_noNop_v4 hcopy
...
/-- LoopSetup code (block 7) is subsumed by divCode_noNop_v4. -/
private theorem divK_loopSetup_code_sub_divCode_noNop_v4 {base : Word} :
    ∀ a i, (divK_loopSetup_code 464 (base + loopSetupOff)) a = some i →
      (divCode_noNop_v4 base) a = some i := by
  intro a i h
  exact sharedNoNop_v4_b7_div a i h
...
/-- BLT singleton at base+loopSetupOff+12 is subsumed by divCode_noNop_v4. -/
private theorem blt_loopSetup_sub_divCode_noNop_v4 {base : Word} :
    ∀ a i, (CodeReq.singleton (base + loopSetupOff + 12) (.BLT .x9 .x0 464)) a = some i →
      (divCode_noNop_v4 base) a = some i := by
  intro a i h
  have hlookup := CodeReq.ofProg_lookup (base + loopSetupOff) (divK_loopSetup 464) 3
    (by decide) (by decide)
  rw [bv64_4mul_3] at hlookup
  exact divK_loopSetup_code_sub_divCode_noNop_v4 a i
    (CodeReq.singleton_mono hlookup a i h)
...
theorem divK_loopSetup_ntaken_spec_within_v4_noNop (sp n v1 v5 : Word) (base : Word)
    (hm_ge : ¬BitVec.slt (signExtend12 (4 : BitVec 12) - n) (0 : Word)) :
    let m := signExtend12 (4 : BitVec 12) - n
    cpsTripleWithin 4 (base + loopSetupOff) (base + loopBodyOff) (divCode_noNop_v4 base)
      (divKLoopSetupNtakenPreNoNop sp v5 v1 n)
      (divKLoopSetupNtakenPostNoNop sp n m) := by
  intro m
  rw [divKLoopSetupNtakenPreNoNop_unfold, divKLoopSetupNtakenPostNoNop_unfold]
  have hbody := divK_loopSetup_body_spec_within sp n v1 v5 464 (base + loopSetupOff)
  have hbodye := cpsTripleWithin_extend_code divK_loopSetup_code_sub_divCode_noNop_v4 hbody
  have hblt_raw := blt_spec_gen_within .x9 .x0 464 m (0 : Word) (base + loopSetupOff + 12)
  rw [show (base + loopSetupOff + 12 : Word) + signExtend13 464 = base + denormOff from by rv64_addr,
      show (base + loopSetupOff + 12 : Word) + 4 = base + loopBodyOff from by bv_addr] at hblt_raw
  have hblt_clean := cpsBranchWithin_ntakenStripPure2 hblt_raw
    (fun hp hQt => by
      obtain ⟨_, _, _, _, _, h_rest⟩ := hQt
      exact absurd ((sepConj_pure_right _).mp h_rest).2 hm_ge)
  have hblte := cpsTripleWithin_extend_code blt_loopSetup_sub_divCode_noNop_v4 hblt_clean
  have hbltef := cpsTripleWithin_frameR
    ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ n) ** ((sp + signExtend12 3984) ↦ₘ n))
    (by pcFree) hblte
  have h12 := cpsTripleWithin_seq_perm_same_cr
