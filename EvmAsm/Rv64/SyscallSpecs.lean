...
@[spec_gen_rv64] theorem slli_spec_gen (rd rs1 : Reg) (v_old v1 : Word) (shamt : BitVec 6)
    (addr : Addr) (hrd_ne_x0 : rd ≠ .x0) :
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .SLLI rd rs1 shamt) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ v_old))
      ((addr ↦ᵢ .SLLI rd rs1 shamt) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ (v1 <<< shamt.toNat))) :=
...
@[spec_gen_rv64] theorem mulhu_spec_gen_rd_eq_rs2 (rd rs1 : Reg) (v1 v2 : Word)
    (addr : Addr) (hrd_ne_x0 : rd ≠ .x0) (hne : rs1 ≠ rd) :
    cpsTriple addr (addr + 4)
      ((addr ↦ᵢ .MULHU rd rs1 rd) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ v2))
      ((addr ↦ᵢ .MULHU rd rs1 rd) ** (rs1 ↦ᵣ v1) ** (rd ↦ᵣ rv64_mulhu v1 v2)) :=
