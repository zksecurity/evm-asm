...
/-- Generic spec for LW: load word from memory.
    Pre:  (base ↦ᵢ .LW rd rs1 offset) ** (rs1 ↦ᵣ v_addr) ** (rd ↦ᵣ v_old) ** (addr ↦ₘ mem_val)
    Post: (base ↦ᵢ .LW rd rs1 offset) ** (rs1 ↦ᵣ v_addr) ** (rd ↦ᵣ mem_val) ** (addr ↦ₘ mem_val)
...
theorem generic_lw_spec (rd rs1 : Reg) (v_addr v_old mem_val : Word)
    ...
    cpsTriple base (base + 4)
      ...
      ((base ↦ᵢ .LW rd rs1 offset) ** (rs1 ↦ᵣ v_addr) ** (rd ↦ᵣ mem_val) ** ((v_addr + signExtend12 offset) ↦ₘ mem_val)) := by
  intro R hR s hPR hpc; subst hpc
...
/-- Generic spec for SW: store word to memory.
    Pre:  (base ↦ᵢ .SW rs1 rs2 offset) ** (rs1 ↦ᵣ v_addr) ** (rs2 ↦ᵣ v_data) ** (addr ↦ₘ mem_old)
    Post: (base ↦ᵢ .SW rs1 rs2 offset) ** (rs1 ↦ᵣ v_addr) ** (rs2 ↦ᵣ v_data) ** (addr ↦ₘ v_data)
    where addr = v_addr + signExtend12 offset -/
theorem generic_sw_spec (rs1 rs2 : Reg) (v_addr v_data mem_old : Word)
    ...
    cpsTriple base (base + 4)
      ...
      ((base ↦ᵢ .SW rs1 rs2 offset) ** (rs1 ↦ᵣ v_addr) ** (rs2 ↦ᵣ v_data) ** ((v_addr + signExtend12 offset) ↦ₘ v_data)) := by
  intro R hR s hPR hpc; subst hpc
...
/-- Generic spec for SW with rs2 = x0: store 0 to memory.
    Pre:  (base ↦ᵢ .SW rs1 .x0 offset) ** (rs1 ↦ᵣ v_addr) ** (addr ↦ₘ mem_old)
    Post: (base ↦ᵢ .SW rs1 .x0 offset) ** (rs1 ↦ᵣ v_addr) ** (addr ↦ₘ 0) -/
theorem generic_sw_x0_spec (rs1 : Reg) (v_addr mem_old : Word)
    (offset : BitVec 12) (base : Addr)
    (hvalid : isValidMemAccess (v_addr + signExtend12 offset) = true) :
    cpsTriple base (base + 4)
      ((base ↦ᵢ .SW rs1 .x0 offset) ** (rs1 ↦ᵣ v_addr) ** ((v_addr + signExtend12 offset) ↦ₘ mem_old))
      ((base ↦ᵢ .SW rs1 .x0 offset) ** (rs1 ↦ᵣ v_addr) ** ((v_addr + signExtend12 offset) ↦ₘ (0 : Word))) := by
  intro R hR s hPR hpc; subst hpc
