...
theorem add_limb0_spec (code : CodeMem) (off_a off_b : BitVec 12)
  ...
  exact cpsTriple_consequence code base (base + 20) _ _ _ _
    (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp)
    (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp) c12345
...
theorem add_limb_carry_spec (code : CodeMem) (off_a off_b : BitVec 12)
  ...
  have h2024 : base + 20 + 4 = base + 24 := by
    apply BitVec.eq_of_toNat_eq; simp [BitVec.toNat_add, BitVec.toNat_ofNat]
  have h2428 : base + 24 + 4 = base + 28 := by
    apply BitVec.eq_of_toNat_eq; simp [BitVec.toNat_add, BitVec.toNat_ofNat]
  have h2832 : base + 28 + 4 = base + 32 := by
    apply BitVec.eq_of_toNat_eq; simp [BitVec.toNat_add, BitVec.toNat_ofNat]
  -- Step 1: LW x7, off_a(x12) — x7 := a_limb
  have s1 := lw_spec_gen code .x7 .x12 sp v7 a_limb off_a base (by nofun) hf1 hvalid_a
  have s1f := cpsTriple_frame_left code base (base + 4) _ _
  ...
  -- Step 5: ADD x7, x7, x5 — x7 := partial + carry_in
  have s5 := add_spec_gen_rd_eq_rs1 code .x7 .x5 (a_limb + b_limb) carry_in (base + 16) (by nofun) (by nofun) hf5
  rw [h1620] at s5
  have s5f := cpsTriple_frame_left code (base + 16) (base + 20) _ _
    ((.x12 ↦ᵣ sp) ** (.x6 ↦ᵣ b_limb) **
     (.x11 ↦ᵣ (if BitVec.ult (a_limb + b_limb) b_limb then (1 : Word) else 0)) **
     ((sp + signExtend12 off_a) ↦ₘ a_limb) ** ((sp + signExtend12 off_b) ↦ₘ b_limb))
    (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _)
     (pcFree_sepConj (pcFree_memIs _ _) (pcFree_memIs _ _))))) s5
  -- Step 6: SLTU x6, x7, x5 — x6 := carry2 (3-reg: rd=x6, rs1=x7, rs2=x5)
  have s6 := sltu_spec_gen code .x6 .x7 .x5 b_limb (a_limb + b_limb + carry_in) carry_in (base + 20) (by nofun) hf6
  rw [h2024] at s6
  ...
  have s7 := or_spec_gen code .x5 .x11 .x6
    ...
    (if BitVec.ult (a_limb + b_limb + carry_in) carry_in then (1 : Word) else 0)
    (base + 24) (by nofun) hf7
  rw [h2428] at s7
  have s7f := cpsTriple_frame_left code (base + 24) (base + 28) _ _
    ((.x12 ↦ᵣ sp) ** (.x7 ↦ᵣ (a_limb + b_limb + carry_in)) **
     ((sp + signExtend12 off_a) ↦ₘ a_limb) ** ((sp + signExtend12 off_b) ↦ₘ b_limb))
    (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _)
     (pcFree_sepConj (pcFree_memIs _ _) (pcFree_memIs _ _)))) s7
  -- Step 8: SW x12, x7, off_b — mem_b := result
  have s8 := sw_spec_gen code .x12 .x7 sp (a_limb + b_limb + carry_in) b_limb off_b (base + 28) hf8 hvalid_b
  rw [h2832] at s8
  have s8f := cpsTriple_frame_left code (base + 28) (base + 32) _ _
    ((.x6 ↦ᵣ (if BitVec.ult (a_limb + b_limb + carry_in) carry_in then (1 : Word) else 0)) **
     (.x5 ↦ᵣ ((if BitVec.ult (a_limb + b_limb) b_limb then (1 : Word) else 0) |||
               (if BitVec.ult (a_limb + b_limb + carry_in) carry_in then (1 : Word) else 0))) **
     (.x11 ↦ᵣ (if BitVec.ult (a_limb + b_limb) b_limb then (1 : Word) else 0)) **
  ...
  have c1234567 := cpsTriple_seq code base (base + 24) (base + 28) _ _ _ c123456
    (cpsTriple_consequence code (base + 24) (base + 28) _ _ _ _
      (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp)
      (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp) s7f)
  have c12345678 := cpsTriple_seq code base (base + 28) (base + 32) _ _ _ c1234567
    (cpsTriple_consequence code (base + 28) (base + 32) _ _ _ _
      (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp)
      (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp) s8f)
  exact cpsTriple_consequence code base (base + 32) _ _ _ _
    (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp)
    (fun _ hp => by simp only [sepConj_assoc', sepConj_comm', sepConj_left_comm'] at hp ⊢; exact hp) c12345678
...
theorem sub_limb_carry_spec (code : CodeMem) (off_a off_b : BitVec 12)
  ...
  have s6 := sub_spec_gen_rd_eq_rs1 code .x7 .x5 (a_limb - b_limb) borrow_in (base + 20) (by nofun) (by nofun) hf6
  rw [h2024] at s6
  have s6f := cpsTriple_frame_left code (base + 20) (base + 24) _ _
    ((.x12 ↦ᵣ sp) ** (.x6 ↦ᵣ (if BitVec.ult (a_limb - b_limb) borrow_in then (1 : Word) else 0)) **
     (.x11 ↦ᵣ (if BitVec.ult a_limb b_limb then (1 : Word) else 0)) **
     ((sp + signExtend12 off_a) ↦ₘ a_limb) ** ((sp + signExtend12 off_b) ↦ₘ b_limb))
    (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _)
     (pcFree_sepConj (pcFree_memIs _ _) (pcFree_memIs _ _))))) s6
  ...
  rw [h2428] at s7
  have s7f := cpsTriple_frame_left code (base + 24) (base + 28) _ _
    ((.x12 ↦ᵣ sp) ** (.x7 ↦ᵣ (a_limb - b_limb - borrow_in)) **
     ((sp + signExtend12 off_a) ↦ₘ a_limb) ** ((sp + signExtend12 off_b) ↦ₘ b_limb))
    (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _)
     (pcFree_sepConj (pcFree_memIs _ _) (pcFree_memIs _ _)))) s7
  -- Step 8: SW x12, x7, off_b
  have s8 := sw_spec_gen code .x12 .x7 sp (a_limb - b_limb - borrow_in) b_limb off_b (base + 28) hf8 hvalid_b
  rw [h2832] at s8
  have s8f := cpsTriple_frame_left code (base + 28) (base + 32) _ _
    ((.x6 ↦ᵣ (if BitVec.ult (a_limb - b_limb) borrow_in then (1 : Word) else 0)) **
     (.x5 ↦ᵣ ((if BitVec.ult a_limb b_limb then (1 : Word) else 0) |||
               (if BitVec.ult (a_limb - b_limb) borrow_in then (1 : Word) else 0))) **
     (.x11 ↦ᵣ (if BitVec.ult a_limb b_limb then (1 : Word) else 0)) **
     ((sp + signExtend12 off_a) ↦ₘ a_limb))
    (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _) (pcFree_sepConj (pcFree_regIs _ _) (pcFree_memIs _ _)))) s8
