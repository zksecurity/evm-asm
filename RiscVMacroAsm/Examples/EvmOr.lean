/-
  RiscVMacroAsm.Examples.EvmOr

  EVM OR opcode implemented and verified as a RISC-V program.

  Source: ethereum/execution-specs Amsterdam fork
    src/ethereum/forks/amsterdam/vm/instructions/bitwise.py:46-68
    ```python
...
/-- OR result: bitwise OR (matches Python `x | y`). -/
def evm_or_result (x y : Word) : Word := x ||| y

-- ============================================================================
-- Section 2: RISC-V Program
-- ============================================================================

/-- OR as a 5-instruction RISC-V program.
    LW   x7,  0(x12)     -- load first operand (top of stack)
    LW   x6,  4(x12)     -- load second operand
    OR   x7, x7, x6      -- compute x7 := a ||| b
...
private def test_state_or : MachineState where
  regs := fun r => match r with
    | .x12 => 0x1000#32  -- stack pointer
    | _ => 0
  mem := fun a =>
    if a == 0x1000#32 then 0xF0#32       -- top of stack = 0xF0
    else if a == 0x1004#32 then 0x0F#32  -- second element = 0x0F
    else 0
  pc := 0

/-- Concrete test: OR(0xF0, 0x0F) = 0xFF -/
example : (stepN 5 (loadProgram 0 evm_or) test_state_or).bind
    (fun s => some (s.getMem 0x1004#32)) = some 0xFF#32 := by native_decide

/-- Concrete test: sp moves from 0x1000 to 0x1004 after OR -/
example : (stepN 5 (loadProgram 0 evm_or) test_state_or).bind
    (fun s => some (s.getReg .x12)) = some 0x1004#32 := by native_decide

private def test_state_or_zero : MachineState where
  regs := fun r => match r with
    | .x12 => 0x1000#32
    | _ => 0
  mem := fun a =>
    if a == 0x1000#32 then 0x00000000#32  -- all zeros
    else if a == 0x1004#32 then 0x00000000#32  -- all zeros
    else 0
  pc := 0

/-- Concrete test: OR(0x00000000, 0x00000000) = 0x00000000 -/
example : (stepN 5 (loadProgram 0 evm_or) test_state_or_zero).bind
...
/-- Low-level CPS spec for evm_or: given fetch hypotheses for 5 instructions,
    transforms
      (x12 ↦ sp, x7 ↦ v7, x6 ↦ v6, sp ↦ₘ a, (sp+4) ↦ₘ b)
    into
      (x12 ↦ sp+4, x7 ↦ a|||b, x6 ↦ b, sp ↦ₘ a, (sp+4) ↦ₘ a|||b). -/
theorem evm_or_spec (code : CodeMem) (a b sp v7 v6 : Word) (base : Addr)
    (hfetch0 : code base = some (Instr.LW .x7 .x12 0))
    (hfetch1 : code (base + 4) = some (Instr.LW .x6 .x12 4))
    (hfetch2 : code (base + 8) = some (Instr.OR .x7 .x7 .x6))
  ...
  have s2 : cpsTriple code (base + 4) (base + 8)
    ...
    exact cpsTriple_consequence code (base + 4) (base + 8) _ _ _ _
      ...
      (fun h hp => by
        ...
        exact hp)
      s2f
  -- Phase 3: OR x7, x7, x6 — base+8 → base+12
  have s3core : cpsTriple code (base + 8) (base + 12)
      ((.x7 ↦ᵣ a) ** (.x6 ↦ᵣ b))
      ((.x7 ↦ᵣ (a ||| b)) ** (.x6 ↦ᵣ b)) := by
    have := or_spec_gen_rd_eq_rs1 code .x7 .x6 a b (base + 8) (by decide) (by decide) hfetch2
...
/-- Stack-level spec: OR pops two elements, pushes their bitwise OR.
    x7 and x6 are clobbered, x12 (sp) moves from sp to sp+4.
    The popped slot (sp ↦ₘ a) is returned as leftover memory. -/
theorem evm_or_stack_spec (code : CodeMem) (a b sp : Word) (rest : List Word)
  ...
  have hlow := evm_or_spec code a b sp v7 v6 base
